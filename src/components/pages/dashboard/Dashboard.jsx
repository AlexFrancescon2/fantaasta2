...
import { DashboardStatsModal } from "./components/stats";
...
export const Dashboard = () => {
  ...
  const onDownloadStrategiesCSV = () => {
    // Get array of rows
    let rows = [
      ["Nome", "Squadra", "Ruolo", "Fascia", "Prezzo massimo", "Tags", "Note"],
    ];
    filteredPlayers.forEach((player) => {
      let tags = "";
      player?.tags.forEach((tagId) => {
        const tag = magheggi.tags.find((tag) => tag.id === tagId);
        if (tag) {
    ...
    dlAnchorElem.setAttribute("download", "strategia.csv");
    dlAnchorElem.click();
  };
        ...
        >
          <div>
            ...
            <Button
              color="black"
              css={{ marginLeft: "15px" }}
              onClick={() => onDownloadStrategiesCSV()}
            >
                        ...
                        >
                          <TableCell padding="small">
                            <Flex onClick={() => setSelectedPlayer(player)}>
                              <Text
                                isBold={owner?.id ? false : true}
                                css={{ "&:hover": { color: "$blue1" } }}
                              >
                                {player.name}
                              </Text>
                            </Flex>
